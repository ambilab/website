...
1. **Run spellcheck**
   - Execute `pnpm spellcheck` to check all `*.{ts,tsx,js,md,mdx,astro,svelte}` files
...
- Files checked: `*.{ts,tsx,js,md,mdx,astro,svelte}`
- Czech content under `src/content/news/cs/` and `src/content/pages/cs/` is excluded via `ignorePaths`
- Compound words are allowed (`allowCompoundWords: true`)
- Common misspellings are flagged via `flagWords` in `cspell.json`
